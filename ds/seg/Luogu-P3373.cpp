...
struct Tag {
    ...
    Tag(LL mult, LL add) : mult(mult % MOD), add(add % MOD) {}

    void apply(Tag tag) {
        mult = mult * tag.mult % MOD;
        add = (add * tag.mult % MOD + tag.add) % MOD;
    }
};
...
struct Item {
    ...
    void apply(Tag tag) {
        x = (x * tag.mult % MOD + tag.add * len % MOD) % MOD;
    }
};
...
int main() {
    ...
    while (q--) {
        ...
        if (cmd == 1) {
            cin >> k;
            lst.update(x, y, Tag(k, 0));
        } else if (cmd == 2) {
            cin >> k;
            lst.update(x, y, Tag(1, k));
        } else {
